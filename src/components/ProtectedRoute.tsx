...
const ProtectedRoute = ({ children }: ProtectedRouteProps) => {
  ...
  // 2. Verificando o estado de autenticação.
  if (!isAuthenticated) {
    // 3. Se 'isAuthenticated' for falso, o usuário não está logado.
  ...
  // 4. Se o usuário estiver logado, renderizamos os componentes filhos normalmente.
  return <>{children}</>;
};
